...
/**
 * =========================================================================
 * HARRISON INTERACTIVE | DEVICE UPLINK MATRIX (INNER ENGINE)
 * =========================================================================
 */
function DeviceLinkContent() {
  ...
  // --- RENDER 1: INVALID OR MISSING TOKEN ---
  if (!token) {
    return (
      <div className="relative z-10 w-full max-w-md glass-panel clip-angled p-10 border-t-4 border-[#DC143C] shadow-[0_0_30px_rgba(220,20,60,0.15)] flex flex-col items-center mx-4 text-center">
        ...
        <p className="font-mono text-xs text-[#8B949E] leading-relaxed">No secure device token provided. Please initiate the link request from inside Unreal Engine 5.</p>
      </div>
    );
  }

  // --- RENDER 2: LOADING SESSION ---
  if (sessionStatus === "loading" || authStatus === "AUTHORIZING") {
    return (
      <div className="relative z-10 w-full max-w-md glass-panel clip-angled p-10 border-t-4 border-[#00BFFF] shadow-[0_0_30px_rgba(0,191,255,0.15)] flex flex-col items-center mx-4 text-center">
        <div className="w-16 h-16 border-4 border-[#00BFFF]/20 border-t-[#00BFFF] rounded-full animate-spin mb-6 shadow-[0_0_15px_rgba(0,191,255,0.3)]"></div>
  ...
  if (sessionStatus === "unauthenticated") {
    return (
      <div className="relative z-10 w-full max-w-md glass-panel clip-angled p-10 border-t-4 border-[#FFBF00] shadow-[0_0_30px_rgba(255,191,0,0.15)] flex flex-col items-center mx-4">
        <div className="absolute top-2 left-2 w-4 h-4 border-t-2 border-l-2 border-[#FFBF00]/50"></div>
        <div className="absolute top-2 right-2 w-4 h-4 border-t-2 border-r-2 border-[#FFBF00]/50"></div>
        <div className="absolute bottom-2 left-2 w-4 h-4 border-b-2 border-l-2 border-[#FFBF00]/50"></div>
        <div className="absolute bottom-2 right-2 w-4 h-4 border-b-2 border-r-2 border-[#FFBF00]/50"></div>
        ...
        <div className="w-full flex flex-col gap-4">
          <button onClick={() => signIn("github")} className="w-full flex items-center justify-between p-4 border border-[#E6EDF3]/20 hover:border-[#E6EDF3]/80 bg-[#E6EDF3]/5 hover:bg-[#E6EDF3]/10 transition-all group clip-angled-button cursor-pointer">
          ...
          <button onClick={() => signIn("google")} className="w-full flex items-center justify-between p-4 border border-[#00BFFF]/30 hover:border-[#00BFFF] bg-[#00BFFF]/5 hover:bg-[#00BFFF]/10 transition-all group clip-angled-button cursor-pointer">
...
/**
 * =========================================================================
 * OUTER SHELL: SUSPENSE & SESSION PROVIDER WRAPPER
 * =========================================================================
 */
      ...
      <div className="min-h-screen w-full flex items-center justify-center relative bg-[#010409] overflow-hidden">
        ...
        <Suspense fallback={
          <div className="relative z-10 flex flex-col items-center">
            <div className="w-12 h-12 border-4 border-[#00BFFF]/20 border-t-[#00BFFF] rounded-full animate-spin shadow-[0_0_15px_rgba(0,191,255,0.3)]"></div>
          </div>
        }>
          <DeviceLinkContent />
        </Suspense>
